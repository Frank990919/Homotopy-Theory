...
If moreover $\catD$ is also cotensored over $\catC$, the module structure is closed. 
...
Presentable categories admit the following stability properties:

\prop{
    For any presentable category $\catC$ and small category $\catI$, the category $\catC^\catI$ is presentable.
}

\prop{[$\dagger$]
    For any presentable category $\catC$ that is also a closed monoidal model category, the category $\cat{Cat}_\catC$ is presentable.
}
